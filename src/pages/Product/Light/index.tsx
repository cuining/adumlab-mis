...
interface BasicProps {
  loading: boolean;
  dispatch: Dispatch;
  light: BasicProfileDataType;
}
interface BasicState {
  visible: boolean;
}

class Light extends Component<BasicProps, BasicState> {
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'light/fetchBasic',
    });
  }

  render() {
    const { light, loading } = this.props;
    const { basicGoods, basicProgress } = light;
...
export default connect(
  ({
    ...
    light,
    loading: loading.effects['light/fetchBasic'],
  }),
)(Light);
